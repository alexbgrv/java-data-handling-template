...
public class SimpleTextService implements TextService {
    ...
    public String toJumpCase(String text) {
        ...
        int sizeOfText = newText.length;
        text = "";
        if (sizeOfText > 0) {
            for (int i = 0; i < sizeOfText; i++) {
                if (i % 2 == 0) {
                    text += Character.toLowerCase(newText[i]);
                } else text += Character.toUpperCase(newText[i]);
            }
        }
        return text;
    }
    ...
    public boolean isPalindrome(String string) {
        ...
        if (sizeOfString <= 1)  return false;
        else {
            for (int i = 0; i < (sizeOfString / 2); ++i) {
                if (string.charAt(i) != string.charAt(sizeOfString - i - 1)) return false;
            }
        }
        return true;
    }
